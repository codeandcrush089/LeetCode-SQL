...
![Easy](https://img.shields.io/badge/Level-Easy-brightgreen?style=for-the-badge) [![Problem Link](https://img.shields.io/badge/LeetCode-Link-orange?style=for-the-badge&logo=leetcode)](https://leetcode.com/problems/bank-account-summary-ii/description/)
...
2. **Join with Users**
   Joining `Users (M rows)` with aggregated result (`K` accounts, where `K ≤ N`)
...
* The subquery `T_Agg` stores **one row per distinct account**, up to `K` rows (`K ≤ N`)
  → **O(K)** extra space.
* Output size is at most `M` rows (one per user) → **O(M)**.

Overall auxiliary space ≈ **O(K)** (dominant from aggregation).
