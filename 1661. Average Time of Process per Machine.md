...
* `Activity AS A_start` & `Activity AS A_end` → Same table used **twice** to match **start** and **end** records.
* `ON machine_id & process_id` → Joins **start and end of the same process on the same machine**.
* `A_end.timestamp - A_start.timestamp` → Calculates **time taken for one process**.
* `AVG(...)` → Finds the **average processing time per machine**.
* `ROUND(..., 3)` → Rounds the result to **3 decimal places**.
* `GROUP BY A_start.machine_id` → Groups calculation **machine-wise**.
