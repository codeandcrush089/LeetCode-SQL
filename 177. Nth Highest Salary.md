...
* The N-th highest salary corresponds to `OFFSET = N - 1`.
  ...
  * Example: N = 1 → offset 0 → highest salary
...
* `DISTINCT salary` → ensures duplicates don’t affect ranking.
* `ORDER BY salary DESC` → sorts from highest to lowest.
* `LIMIT 1 OFFSET offset_val` → picks exactly the **Nth** highest salary.
* If N is larger than the number of distinct salaries → returns **NULL**, which is correct.
