...
* **Step 1 (T1 – CROSS JOIN Students × Subjects)**
...
* **Step 2 (T2 – aggregate exam counts)**
...
* **Step 3 (LEFT JOIN + COALESCE)**
  ...
  * Joins T1 with T2.
  * If a student has **no matching exam record**, `attended_exams` will be NULL → replaced with **0** via `COALESCE`.
