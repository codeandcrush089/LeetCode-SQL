...
### **1️⃣ First subquery → User who rated the most movies**
...
* Sorts:
  ...
  * Most ratings **first**
  * If tied → **alphabetically smallest name**
* `LIMIT 1` → picks exactly one user
* Returns the user's **name** as `results`



### **2️⃣ Second subquery → Highest-rated movie in Feb 2020**

* Filters ratings to **2020-02**
* Joins `Movies` with `MovieRating`
* Calculates each movie’s **average rating for that month**
* Sorts:

  * Highest average rating first
  * If tied → **alphabetically smallest title**
* `LIMIT 1` → picks the top movie
