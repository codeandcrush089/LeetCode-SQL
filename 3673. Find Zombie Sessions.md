...
![Hard](https://img.shields.io/badge/Level-Hard-red?style=for-the-badge) [![Problem Link](https://img.shields.io/badge/LeetCode-Link-orange?style=for-the-badge&logo=leetcode)](https://leetcode.com/problems/find-zombie-sessions/)
...
---
## 🧐 Query Explanation
* `GROUP BY session_id, user_id` → Work **per session per user**.

* `TIMESTAMPDIFF(MINUTE, MIN(event_timestamp), MAX(event_timestamp))`
  → Time between first and last event = **session duration in minutes**.

* `SUM(CASE WHEN event_type = 'scroll' THEN 1 ELSE 0 END) AS scroll_count`
  → **Total scroll events** in that session.
...
3. Click-to-scroll ratio < 0.20
   → clicks / scrolls `< 0.20` (using `NULLIF` to avoid divide-by-zero).

4. No purchases
   → `SUM(event_type = 'purchase') = 0`

`ORDER BY scroll_count DESC, session_id ASC`
→ Sessions with **most scrolls first**, then by session_id.
