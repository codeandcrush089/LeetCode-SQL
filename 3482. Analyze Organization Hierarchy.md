...
![Hard](https://img.shields.io/badge/Level-Hard-red?style=for-the-badge) [![Problem Link](https://img.shields.io/badge/LeetCode-Link-orange?style=for-the-badge&logo=leetcode)](https://leetcode.com/problems/analyze-organization-hierarchy/description/)
...
* **Hierarchy CTE**
  ...
  * Recursively adds employees whose `manager_id = previous employee_id`, increasing `level` each time → gives **each employee’s level in the hierarchy**.

* **TeamMetrics CTE**

  * First part: for each `manager_id`, counts how many employees in `Hierarchy` have that manager →
...
* **Final SELECT**
  ...
  * `team_size` = `SUM(tm.team_size)` → total **direct + indirect reports** under that employee.
  * `budget` = `own salary + SUM(reports_salary_budget)` → **total salary cost of the whole team**, including the manager.
