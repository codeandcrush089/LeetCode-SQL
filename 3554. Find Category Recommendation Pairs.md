...
* **`UserCategories` CTE**
  ...
  * Gets **unique (user_id, category)** pairs → which categories each user has ever purchased.
...
* **Final SELECT**
  ...
  * `COUNT(DISTINCT user_id)` → Counts **how many different customers** bought **both categories** in the pair.
  * `HAVING COUNT(DISTINCT user_id) >= 3` → Keep only **popular pairs** (at least 3 customers).
  * `ORDER BY customer_count DESC, category1, category2` → Most common pairs first.
