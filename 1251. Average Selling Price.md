...
* `p.price * u.units` → Calculates **revenue** for each sale during that price period.

* `SUM(p.price * u.units)` → Total revenue for the product across all price periods.

* `SUM(u.units)` → Total **units sold**.

* `SUM(revenue) / SUM(units)` → **Weighted average price**, based on how many units were sold at each price.

* `COALESCE(..., 0.00)` → If no units were sold (SUM = NULL), return **0**.

* `LEFT JOIN UnitsSold`

  * Keeps all price records, even if the product had no sales.
  * Includes only sales where the **purchase date is within the price’s start–end date range**.

* `GROUP BY p.product_id` → One result per product.
