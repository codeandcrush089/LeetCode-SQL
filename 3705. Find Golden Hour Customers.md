...
* `SUM(CASE WHEN HOUR(order_timestamp) BETWEEN 11 AND 13 OR 18 AND 20 THEN 1 ELSE 0 END)`
  → Counts how many orders were placed in **peak hours** (lunch + dinner).

* `ROUND((that_count * 100.0) / COUNT(order_id), 0) AS peak_hour_percentage`
  → Percentage of the customer’s orders that are in **peak hours**.

* `ROUND(AVG(order_rating), 2) AS average_rating`
  → Average rating given by the customer (ignoring NULLs).

**HAVING conditions (filters):**

1. `COUNT(order_id) >= 3` → Customer has **at least 3 orders**.
2. Peak orders ≥ 60% → They **mostly order in peak hours**.
3. `AVG(order_rating) >= 4.0` → Their **average rating is at least 4 stars**.
4. Rated orders ≥ 50% → They have **rated at least half of their orders**.

`ORDER BY average_rating DESC, customer_id DESC` → Best raters first, then higher customer_id.
