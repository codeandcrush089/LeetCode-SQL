...
* `ROW_NUMBER() OVER(PARTITION BY company ORDER BY salary)`
  → Assigns a **salary rank within each company** (lowest to highest).

* `COUNT(*) OVER(PARTITION BY company)`
  → Gets the **total number of employees per company**.

* `WHERE row_number >= count/2 AND row_number <= count/2 + 1`
  → Selects the **middle one (odd count)** or **two middle employees (even count)**.

* `SELECT id, company, salary`
  → Returns only the **employee ID, company, and salary**.
