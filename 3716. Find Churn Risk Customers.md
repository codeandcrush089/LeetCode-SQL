...
![Medium](https://img.shields.io/badge/Level-Medium-yellow?style=for-the-badge) [![Problem Link](https://img.shields.io/badge/LeetCode-Link-orange?style=for-the-badge&logo=leetcode)](https://leetcode.com/problems/find-churn-risk-customers/)
...
---
## 🧐 Query Explanation
* `user_with_last_event` →
  Adds a row number per `user_id` ordered by **latest event_date + event_id**, so `rn = 1` = **most recent event** for each user.

* `user_history` → Per `user_id`, calculates:

  * `start_date` → **first event date**
  * `last_event_date` → **most recent event date**
  * `max_historical_amount` → **highest monthly_amount ever**
  * `downgrade_count` → how many times event_type = **'downgrade'**

* `latest_event` → Picks only `rn = 1` from first CTE = **each user’s latest event**, with:
...
* Final `SELECT`:
  ...
  * `DATEDIFF(h.last_event_date, h.start_date)` → total **days as subscriber**.
...
* `ORDER BY days_as_subscriber DESC, user_id ASC` → Longest subscribers first, then by user_id.
