...
![Hard](https://img.shields.io/badge/Level-Hard-red?style=for-the-badge) [![Problem Link](https://img.shields.io/badge/LeetCode-Link-orange?style=for-the-badge&logo=leetcode)](https://leetcode.com/problems/first-letter-capitalization-ii/description/)
...
* **`Words` CTE (recursive)**
  ...
  * Breaks `content_text` into **one word per row**.
  * `token_index` keeps the **original word order**.
  * Recursion continues until `remaining_text` becomes empty.

* **`Converted` CTE**

  * Rebuilds the sentence using `GROUP_CONCAT` ordered by `token_index`.
  * For **normal words** (no hyphen):

    * `Upper(first letter) + lower(rest)` → `Hello`, `World`.
  * For **hyphenated words** (like `data-driven`):

    * Capitalizes both parts → `Data-Driven`.

* **Final SELECT**

  * Shows `content_id`, original text, and the **converted (properly capitalized) text** for each row.
